...
% If not, we continue increasing the penalty parameters slower, limiting the increase to the  
% value of the regularization threshold.
% ------------------------------------------------------------------------------------------------ %
    cond_p = 0.95*norm(res_p) > norm(new_res_p);
    cond_d = 0.95*norm(res_d) > norm(new_res_d);
    if (cond_p || cond_d)
        beta = min(reg_limit,beta*1.2);              % Standard PMM (heuristic)  
    else
